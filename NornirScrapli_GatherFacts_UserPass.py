...
def gather_router_facts(task, result_dict):
    ...
    with IOSXEDriver(
        ...
        auth_username=task.host.get("username", ""),  # Use default if not defined in inventory
        auth_password=task.host.get("password", ""),  # Use default if not defined in inventory
